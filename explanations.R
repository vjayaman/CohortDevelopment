...
blurb <- function(a = NULL, type) {
  if (type == "FacetedPlot") {
    ...
    paste0("Click on a row in the table below to see the specifics of the clusters ", 
      ...
      paste0("We have two pairs of slider inputs. Consider first positive homogeneity.", 
        "In the first slider input, we select a percentage between 0 and 100, call this x. ", 
        "Directly below this input, we select a step size, s1. This will tell us the increment ", 
        "size, so we can see at which values we run the calculations. For example, if we pick ", 
        "x = 80 and s1 = 5 for the slider inputs for positive homogeneity, then we ", 
        "look for clusters at least as large as ", inp$minC, ", where at least 80% of the cluster", 
        " has the limiting factor, then we repeat for 85%, 90%, 95%, and 100%. These are the size and ", 
        "homogeneity conditions. In the negative homogeneity case, we again select a percentage between ", 
        "0 and 100, call this y, and a step size s2. Suppose we say y = 30 and s2 = 10. Then we look ", 
        "for clusters at least as large as ", inp$minC, ", where no more than 30% of the cluster has ", 
        "the limiting factor, and then repeat for 20% and 10%. Then, we select whether we want to look ", 
        "at the number of such clusters or the proportion of the population found in such clusters. In the ", 
        "former case, the plot below will show, at each height, the number of clusters that satisfy the ", 
        "described conditions. In the latter scenario, we can see the proportion of the entire population ", 
        "found in clusters satisfying the size and homogeneity conditions.") %>% return()
  }
}
...
output$base_metrics <- renderText({
  ...
  paste0("Dataset size: ", nrow(inp$data), " samples\n", 
         ...
         a1$Type[p], " (", a1$Bin[p], "): ", a1$Freq[p], "/", a1$Tot[1], " = ", a1$percent[p], "\n    ", 
