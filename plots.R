...
output$select_height <- renderUI({
  req(user$initial, user$ptype, best$heights)
  tagList(
...
output$bubble_plot <- renderPlot({
  req(user$results, plots$bubble_data, plots$bubble_title, inp$limiting)
...
output$positive_bubble <- renderPlotly({
  req("pos_plot" %in% input$int_plots, user$results, plots$bubble_data)
  ...
  {ggplot(df_pos, aes(x = `Cluster names`, y = `Percent of cluster`, color = `Percent interval`)) + 
      ...
      ggtitle(paste0("Positive cluster homogeneity (>= y % of the cluster has ", tolower(inp$limiting), ")")) + 
      scale_color_manual(values = pos(p1), name = "Percent\ninterval")} %>% 
    ggplotly()
})

output$negative_bubble <- renderPlotly({
  req("neg_plot" %in% input$int_plots, user$results, plots$bubble_data)
  ...
  {ggplot(df_neg, aes(x = `Cluster names`, y = `Percent of cluster`, color = `Percent interval`)) + 
      geom_point() + geom_point(aes(size = `Cluster size`), show.legend = FALSE)  + 
      ggtitle(paste0("Negative cluster homogeneity (<= y % of the cluster has ", tolower(inp$limiting), ")")) +
